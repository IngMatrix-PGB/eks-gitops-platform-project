...
es_count="$(pkubectl get crd -o name | grep -c '\.external-secrets\.io$' || true)"
gen_count="$(pkubectl get crd -o name | grep -c '\.generators\.external-secrets\.io$' || true)"
...
echo "test-secret-lifecycle: reconciling GitOps from branch '$current_branch' ..."
...
# --- three-way hash comparison, never printing the value ---
three_way_hash() {
  # $1=env source_ns/source_secret/target_ns/target_secret/pod_selector
  ns="$1"; target_secret="$2"; pod_label_ns="$3"
  target_sha="$(pkubectl get secret "$target_secret" -n "$ns" -o jsonpath='{.data.message}' | base64 -d | shasum -a 256 | awk '{print $1}')"
  pod_name="$(pkubectl get pod -n "$pod_label_ns" -l "app.kubernetes.io/instance=platform-smoke-${ns}" -o jsonpath='{.items[0].metadata.name}')"
  mounted_sha="$(pkubectl exec -n "$pod_label_ns" "$pod_name" -- sha256sum /etc/secret/message 2>/dev/null | awk '{print $1}')"
  echo "${target_sha} ${mounted_sha} ${pod_name}"
}

echo "test-secret-lifecycle: comparing source/target/mounted-file hashes ..."
read -r staging_target_sha_1 staging_mounted_sha_1 staging_pod_1 <<EOF
$(three_way_hash staging "$staging_target" staging)
EOF
read -r production_target_sha_1 production_mounted_sha_1 production_pod_1 <<EOF
$(three_way_hash production "$production_target" production)
EOF

if [ "$staging_source_sha" = "$staging_target_sha_1" ] && [ "$staging_target_sha_1" = "$staging_mounted_sha_1" ]; then
  echo "OK: staging source/target/mounted-file hashes all match ($staging_target_sha_1)"
else
  echo "FAIL: staging three-way hash mismatch (source=$staging_source_sha target=$staging_target_sha_1 mounted=$staging_mounted_sha_1)" >&2
  fail=1
fi
if [ "$production_source_sha" = "$production_target_sha_1" ] && [ "$production_target_sha_1" = "$production_mounted_sha_1" ]; then
  echo "OK: production source/target/mounted-file hashes all match ($production_target_sha_1)"
else
  echo "FAIL: production three-way hash mismatch (source=$production_source_sha target=$production_target_sha_1 mounted=$production_mounted_sha_1)" >&2
  fail=1
fi
...
# --- 20/21: restore persistent state - ESO reinstalled, GitOps
# reconciled from main. The source secrets provisioned in step 5/13
# are NOT deleted: this feature is now the persistent, merged design -
# main's own values-staging.yaml/values-production.yaml enable
# externalSecret, so the source secrets must keep existing for the
# ExternalSecrets to stay Ready after this test ends. ---
echo "test-secret-lifecycle: restoring persistent state (ESO reinstall + gitops-bootstrap REVISION=main) ..."
sh lab/eso/install.sh
make gitops-bootstrap REVISION=main
pkubectl annotate application platform-bootstrap -n argocd argocd.argoproj.io/refresh=hard --overwrite >/dev/null 2>&1 || true

restored=0
elapsed=0
while [ "$elapsed" -lt 90 ]; do
  s1="$(pkubectl get externalsecret platform-smoke-staging-standard-workload-secret -n staging -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || true)"
  s2="$(pkubectl get externalsecret platform-smoke-production-standard-workload-secret -n production -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || true)"
...
if [ "$restored" -eq 1 ]; then
  echo "OK: persistent state restored from main - both ExternalSecrets Ready"
else
  echo "FAIL: persistent state did not restore to both ExternalSecrets Ready within 90s" >&2
  fail=1
fi
